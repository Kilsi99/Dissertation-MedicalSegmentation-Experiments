# -*- coding: utf-8 -*-
"""Skin_leshion_CBAM.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1cQJjg2FP2qnTAo2HNaYdxkNvnNo8dfcs
"""
...
img, mask = load_img_mask(case_list[120])
...
display_label(img, mask)

def overlay_mask(img, mask, color=[255, 0, 0], alpha=0.4, scale=1.0):
    """
    Overlays a mask on an image.

    Parameters:
    - img: np.array, RGB image
    - mask: np.array, binary mask (0 for background, 1 for foreground)
    - color: list, RGB color for the overlay
    - alpha: float, transparency of the overlay (0=transparent, 1=solid)
    - scale: float, scaling factor for display

    Returns:
    - resized image with overlay applied
    """
    # Ensure binary mask
    ...
    # Blend only on masked areas
    blended = img.copy()
    blended[mask == 1] = cv2.addWeighted(img[mask == 1], 1 - alpha, overlay[mask == 1], alpha, 0)

    # Resize for display
    height, width = blended.shape[:2]
    new_size = (int(scale * width), int(scale * height))
    resized = cv2.resize(blended, new_size)
...
def plot_accuracy(train_acc, val_acc, title='Training and validation loss over epochs: CBAM_model'):
...
def plot_loss(train_loss, val_loss, title='Training and validation loss over epochs: CBAM_model'):
...
def plot_dice(train_dice, val_dice, title='Training and validation loss over epochs: CBAM_model'):
...
def save_model(model, save_dir, filename="CBAM_model2.pth"):
...
save_model(CBAM_model, '/content/drive/MyDrive/Dissertation/Skin_lesion_segmentation/models')

CBAM_model = full_Attention_unet(3,32)
CBAM_model.load_state_dict(torch.load('/content/drive/MyDrive/Dissertation/Skin_lesion_segmentation/models/CBAM_model2.pth', map_location= torch.device('cpu')))
...
masks_imgs_list = [predict_and_visualize_mask(c, CBAM_model, device, test_img_transform, test_image_dir, test_mask_dir) for c in case_list_num]
