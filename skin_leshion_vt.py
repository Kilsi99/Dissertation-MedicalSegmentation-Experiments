# -*- coding: utf-8 -*-
"""Skin_leshion_VT.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1L5cAsR28zkbBtqRg990YLczvhWZqfU_G
"""
...
VT_model = VisionTransformerSegmentation(
    channels_in=3,
    emb_size=128,
    patch_size=(8,8),
    img_size=(256,256),
...
criterion = multi_fn_loss(0.95, 0.5)
...
def save_model(model, save_dir, filename="VT_model.pth"):
...
save_model(VT_model, '/content/drive/MyDrive/Dissertation/Skin_lesion_segmentation/models')

VT_model = VisionTransformerSegmentation(
    channels_in=3,
    emb_size=128,
    patch_size=(8,8),
    img_size=(32,32), # Corrected image size
    base=10000,
    num_heads=8,
    n_layers=3,
    num_classes=2  # background and lesion
)
VT_model.load_state_dict(torch.load('/content/drive/MyDrive/Dissertation/Skin_lesion_segmentation/models/VT_model.pth', map_location= torch.device('cpu')))
...
def test_model(model, test_loader, criterion, device):
    ...
    dice_calculator = Dice_coef().to(device) # Renamed variable
    accuracy = []
    loss_list = []
    dice_list = []
    ...
    with torch.no_grad():
        for img, masks in test_loader:
            ...
            loss = criterion(outputs, masks)
            acc = calculate_accuracy(outputs, masks)
            dice = dice_calculator(outputs, masks) # Use the renamed variable
...
masks_imgs_list = [predict_and_visualize_mask(c, VT_model, device, test_img_transform, test_image_dir, test_mask_dir) for c in case_list_num]
