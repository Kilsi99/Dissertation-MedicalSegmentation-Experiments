# -*- coding: utf-8 -*-
"""Skin_leshion_local_VT.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1r9HnHFSfHqQ--QWYOKl1h1wtByESEEzV
"""
...
VT_local_model = VisionTransformerLocalAttention(
    channels_in=3,
    emb_size=128,
    patch_size=(8,8),
    img_size=(256,256),
...
optimiser = torch.optim.Adam(VT_local_model.parameters(), lr=1e-4)

VT_local_metrics = train_model(VT_local_model, train_loader, val_loader, criterion, optimiser, device, 25)

def plot_accuracy(train_acc, val_acc, title='Training and validation loss over epochs: VT_local_model'):
...
def plot_loss(train_loss, val_loss, title='Training and validation loss over epochs: VT_local_model'):
...
def plot_dice(train_dice, val_dice, title='Training and validation loss over epochs: VT_local_model'):
...
plot_accuracy(VT_local_metrics['train_accuracy'],VT_local_metrics['val_accuracy'])
plot_loss(VT_local_metrics['train_loss'], VT_local_metrics['val_loss'])
plot_dice(VT_local_metrics['Train_dice'], VT_local_metrics['val_dice'])
...
save_model(VT_local_model, '/content/drive/MyDrive/Dissertation/Skin_lesion_segmentation/models')
...
VT_local_model.load_state_dict(torch.load('/content/drive/MyDrive/Dissertation/Skin_lesion_segmentation/models/VT_local_model.pth', map_location= torch.device('cpu')))
...
test_model(VT_local_model, test_loader, criterion, device)
...
test_model_metrics_skimage(VT_local_model, test_loader, device)
...
masks_imgs_list = [predict_and_visualize_mask(c, VT_local_model, device, test_img_transform, test_image_dir, test_mask_dir) for c in case_list_num]
